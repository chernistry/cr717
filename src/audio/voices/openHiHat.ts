export interface OHParams {
  level?: number;
  accent?: number;
}

const defaults: Required<OHParams> = {
  level: 0.7,
  accent: 0,
};
...
export function playOH(ctx: AudioContext, time: number, params: OHParams = {}): void {
  ...
  gain.gain.exponentialRampToValueAtTime(0.0001, time + 0.49);
